...
\chapter{Differentiation in $\mathbb{R}^n$}
		...
		\begin{definition}[Directional Derivative]
			Let $A \subseteq \R^m$ and let $f : A \to \R^n$. Suppose that $A$ contains a neighbourhood of $a = (a_1, \dots, a_m)$. Given $u= (u_1, \dots, u_m) \in \R^m$ with $u  \neq 0 \in \R^n$ define $$f'_a(u) = \lim_{t \to 0} \phi_u(t)$$ (provided the limit exists) where $\phi_u : \{ t \in \R \setminus \{0\} \ | \ a+tu \in A\} \to \R^n$ defined by $$\phi_u(t) = \lim_{t\to 0}\frac{f(a+tu)-f(a)}{t}$$
		\end{definition}
		
		Now let $V$ be a neighborhood of $a$ then there exists an $r > 0$ such that $$(a_1 -r, a_1 + r) \times \dots \times (a_m -r, a_m+r) \subseteq V \subseteq A$$
		
		\begin{theorem}
			Let $R = \max\{|u_1|, \dots, |u_m|\}$ then the open set $$\left(-\frac{r}{R}, \frac{r}{R} \right) \setminus \{0\}$$ is a set on which $\phi_u$ is well defined.
		\end{theorem}
		
			\begin{proof}
				Pick $i \in \{1, \dots, m\}$. We claim that $a_i + tu_i \in (a_r-r, a_i+r)$. We have two cases to examine $u_i \geq 0$ or $u_i < 0$. Suppose that $u_i \geq 0$ then $$a_i -r = a_i - \frac{r}{|u_i|}u_i \leq a_i - \frac{r}{R}u_i < a_i + tu_i < a_i + \frac{r}{R}u_i \leq a_i + \frac{r}{|u_i|}u_i = a_i + r.$$
				 if $u_i < 0$ then 
				 \begin{align*}
				 ...
				 &= a_i + r
				 \end{align*}
				 In both cases we have $$a_i -r <a_i + tu_i < a_i + r.$$ Thus $a_i + tu_i \in (a_i-r, a_i+r)$.`
			\end{proof}
